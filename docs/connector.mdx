...
C1 reads each Zoom user's groups, role, and license tier from `GET /v2/users/{userId}` during the user grants pass. The account profile retains the Zoom license `type`, but does not store group or role membership. Group administrator assignments come from each group's administrator list. Zoom does not expose a `/licenses` endpoint, so license tiers are modeled from each user's Zoom license type.
