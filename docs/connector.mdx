...
og:description: C1 provides identity governance and just-in-time provisioning for Zoom. Integrate your Zoom instance with C1 to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access.
description: C1 provides identity governance and just-in-time provisioning for Zoom. Integrate your Zoom instance with C1 to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access.
sidebarTitle: Zoom
---
...
A Zoom user holds one account role and one license tier, and both are exclusive. Granting a role replaces the role the user already holds, and granting a license tier replaces the tier they are on. C1 marks the superseded grant revoked. Revoking a license tier returns the user to Basic, which is Zoom's floor tier and consumes no seat, because Zoom has no unlicensed state. Group administrator is separate from the account role, and one user can administer several groups.

A contact group member is either a user or another user group. When a user group is a member, the users in that group also hold the contact group entitlement.
